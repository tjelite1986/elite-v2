...
export async function createGateToken(userId: string): Promise<string> {
  // The scope string is an opaque token claim, not a section name: it is kept
  // as "shorts18" because changing it invalidates every gate cookie in the wild
  // and makes everyone re-enter their PIN for no gain.
