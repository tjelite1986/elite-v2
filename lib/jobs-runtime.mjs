...
export const JOBS = [
  ...
  {
    id: "videos-scan",
    name: "Video library scan",
    description:
      "Mirror VIDEOS_ROOT/{main,adults} into the library: add new files, refresh changed ones, generate posters/storyboards, drop rows whose file is gone.",
    defaultIntervalSeconds: 3600,
    run: {
      kind: "http",
  ...
  },
  {
    ...
    description:
      "Convert library videos a browser cannot play (DVD rips, HEVC, AC-3, .mkv/.avi) to H.264/AAC MP4, replacing the source. Starts the run and returns — an encode outlives any HTTP request — so 'ok' here means started, not finished. Works the queue within a time budget, spreading a big backlog over several runs.",
    defaultIntervalSeconds: 3600,
    run: {
      kind: "http",
  ...
  {
    id: "videos-metadata",
    name: "Video metadata match",
    description:
      "Match 18+ library videos to their metadata: a .nfo sidecar next to the file when there is one (Whisparr/Stash layouts), otherwise a ThePornDB search that only auto-applies confident matches. Starts the run and returns; unmatched files stay available for manual matching.",
    defaultIntervalSeconds: 3600,
    run: {
      kind: "http",
  ...
  {
    id: "videos-summary",
    name: "Video AI summary",
    description:
      "Describe library videos from their storyboard sheet using a vision model, storing a short summary and keywords per video. One API call per video (no local CPU), so it runs happily alongside a transcode. Main channel only by default — the 18+ library is not sent to a third-party API. Requires ANTHROPIC_API_KEY; starts the run and returns.",
    defaultIntervalSeconds: 3600,
    run: {
      kind: "http",
      path: "/api/videos/summarize",
      secretHeader: "x-import-secret",
      secretEnv: "IMPORT_CRON_SECRET",
    },
  },
  {
    id: "shorts-summary",
    name: "Shorts AI summary",
    description:
      "Describe shorts from their frames using a vision model, storing a short summary and keywords per clip. Shorts have no stored storyboard, so a contact sheet is sampled from the clip at describe time — one ffmpeg pass plus one API call each. Main channel only by default; the 18+ channel is not sent to a third-party API. Requires an AI key; starts the run and returns.",
    defaultIntervalSeconds: 3600,
    run: {
      kind: "http",
      path: "/api/shorts/summarize",
      secretHeader: "x-import-secret",
      secretEnv: "IMPORT_CRON_SECRET",
    },
  },
  {
    ...
    name: "Clip fingerprints",
    description:
      "Fingerprint shorts and library videos for duplicate detection: sixteen frames sampled across each clip, reduced to a perceptual hash plus a colour signature. ffmpeg only, no API calls, so it is free to run over the whole library — but it does use CPU, so it shares the machine with the transcode queue.",
    defaultIntervalSeconds: 21600,
    run: {
      kind: "http",
      path: "/api/media/fingerprint",
      secretHeader: "x-import-secret",
      secretEnv: "IMPORT_CRON_SECRET",
    },
  },
  ...
  {
    id: "user-import",
    name: "User folder import",
    description:
      "Import files dropped in the per-user _import trees (u_<user>/{gallery,posts,shorts,shorts18,books}).",
    defaultIntervalSeconds: 900,
    run: {
      kind: "http",
      path: "/api/import/user-folders",
      secretHeader: "x-import-secret",
      secretEnv: "IMPORT_CRON_SECRET",
    },
  },
  {
    id: "import-normalize",
    name: "Import image normalize",
    description:
      "Convert HEIC images and fix mislabeled extensions in the user import folders so imports never fail on them.",
    defaultIntervalSeconds: 3600,
    run: { kind: "script", script: "scripts/normalize-import-images.mjs" },
  },
  {
    id: "posts-dupescan",
  ...
  {
    id: "appstore-import",
    name: "App Store folder import",
    description:
      "Import APK files dropped in the app store import folder; unmatched files go to the review queue.",
    defaultIntervalSeconds: 300,
    run: {
      kind: "http",
      path: "/api/store/admin/import/folder",
      secretHeader: "x-import-secret",
      secretEnv: "IMPORT_CRON_SECRET",
    },
  },
  {
    id: "telegram-appstore",
    name: "Telegram APK sync",
    description:
      "Download new .apk/.xapk posts from the configured Telegram channels into the app store import folder.",
    defaultIntervalSeconds: 900,
    run: { kind: "script", script: "scripts/telegram-appstore-sync.mjs" },
  },
  {
    id: "instagram-sync",
    name: "Instagram sync",
...
function runScript(rel, extraEnv) {
  return new Promise((resolve) => {
    ...
    const child = spawn(process.execPath, ["--max-old-space-size=768", rel], {
      cwd: PROJECT_ROOT,
