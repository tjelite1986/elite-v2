...
export const JOBS = [
  ...
  // The shorts jobs left with the library: the main channel moved to its own
  // app on 2026-08-31 and the 18+ channel on 2026-09-15, and each of those apps
  // runs its own importer, poller, transcoder, duplicate scan and cleanup. A job
  // kept here would either do nothing or, worse, re-create a library this app no
  // longer serves — which is how the retired main channel refilled itself once.
  // Their rows in job_schedules are disabled rather than dropped, so the history
  // of what ran stays readable.
  {
    id: "videos-scan",
  ...
  },
  {
    ...
    description:
      "Fingerprint the library videos for duplicate detection: sixteen frames sampled across each clip, reduced to a perceptual hash plus a colour signature. ffmpeg only, no API calls, so it is free to run over the whole library — but it does use CPU, so it shares the machine with the transcode queue.",
  ...
  },
  {
    ...
    description:
      "Import files dropped in the per-user _import trees (u_<user>/{gallery,posts,books}). The two shorts sections are gone: those libraries are separate apps, each with a drop folder of its own.",
    defaultIntervalSeconds: 900,
    run: {
      kind: "http",
