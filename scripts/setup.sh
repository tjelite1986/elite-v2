...
# Locate the elite-v2 repo root (Dockerfile + package.json): first where this
# script lives, then the current directory. Empty if neither matches — e.g.
# when a copy of the script runs outside the repo.
detect_repo_dir() {
  local d
  d="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." 2>/dev/null && pwd || true)"
  if [[ -n "$d" && -f "$d/Dockerfile" && -f "$d/package.json" ]]; then printf '%s' "$d"; return 0; fi
  if [[ -f ./Dockerfile && -f ./package.json ]]; then pwd; return 0; fi
  printf ''
}
...
write_compose() {
  ...
  # Build context: "." when generating inside the repo, otherwise the detected
  # (or asked-for) repo path, so the compose file works from any directory.
  local ctx="."
  if [[ ! -f ./Dockerfile || ! -f ./package.json ]]; then
    ctx="$(ask "elite-v2 repo path (docker build context)" "${REPO_DIR:-/opt/elite-v2}")"
    if [[ ! -f "$ctx/Dockerfile" ]]; then
      say ">> Warning: no Dockerfile in $ctx — fix the compose context before building."
    fi
  fi
