...
export default async function ShortsProfilePage({
  ...
  // 18+ profiles live in the separate /shorts18 section (which gates access);
  // the main section never renders them.
  if (profile.channel === "18plus") {
    redirect(`/shorts18/profile/${profile.id}`);
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-2 pb-24 pt-16 text-white">
      <div className="mb-4 flex items-center gap-2 px-1">
        ...
        {session.role === "admin" && (
          <Link
            href={`/shorts/profile/${profile.id}/candidates`}
